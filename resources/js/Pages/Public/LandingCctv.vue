...
<template>
  <Head :title="landing?.content?.seo_title || `${landing.name} — CCTV`" />
  <div class="min-h-screen bg-base-200">
    <div class="navbar bg-base-100 shadow-sm">
      <div class="flex-1 px-4 font-semibold">{{ landing.name }}</div>
    </div>

    <div class="hero min-h-[70vh] bg-gradient-to-br from-secondary/15 to-base-200">
      <div class="hero-content text-center">
        <div class="max-w-xl">
          <h1 class="text-4xl font-bold tracking-tight">
            {{ landing?.content?.headline || 'CCTV & solusi jaringan' }}
          </h1>
          ...
          <p class="mt-4 text-base-content/80">
            {{ landing?.content?.body || 'Landing untuk bisnis instalasi / project. Warehouse default untuk stok material:' }}
