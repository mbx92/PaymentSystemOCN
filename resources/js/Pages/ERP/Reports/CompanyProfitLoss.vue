...
const reload = () => {
  router.get(route('reports.company-profit-loss'), {
...
<template>
  <Head title="Laba Rugi per Usaha" />
  <AppLayout>
    <div class="space-y-5">
      <div class="ocn-panel">
        <div class="ocn-panel__head">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <h1 class="ocn-panel__title mt-1">Laba Rugi per Usaha</h1>
              <p class="ocn-panel__desc mt-1">Ringkasan pendapatan, beban, dan net profit tiap usaha berdasarkan jurnal yang sudah diposting.</p>
            </div>
      ...
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="ocn-panel">
          <div class="ocn-panel__head py-3"><h2 class="ocn-panel__title text-sm font-medium">Total beban</h2></div>
          <div class="card-body py-4"><p class="text-xl font-bold text-error">{{ format(totals?.expense ?? 0) }}</p></div>
        </div>
        <div class="ocn-panel">
          <div class="ocn-panel__head py-3"><h2 class="ocn-panel__title text-sm font-medium">Net profit</h2></div>
          <div class="card-body py-4">
            <p class="text-xl font-bold" :class="(totals?.net_profit ?? 0) >= 0 ? 'text-primary' : 'text-error'">{{ format(totals?.net_profit ?? 0) }}</p>
          </div>
        </div>
        <div class="ocn-panel">
      ...
      <div class="ocn-panel">
        <div class="ocn-panel__head">
          <h2 class="ocn-panel__title">Ringkasan laba rugi per usaha</h2>
        </div>
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="row in rows ?? []" :key="row.company_id ?? row.company_name">
                ...
                <td class="text-right text-success">{{ format(row.revenue_total) }}</td>
                <td class="text-right text-error">{{ format(row.expense_total) }}</td>
                <td class="text-right font-semibold" :class="row.net_profit >= 0 ? 'text-primary' : 'text-error'">{{ format(row.net_profit) }}</td>
                <td class="text-right">{{ row.entry_count }}</td>
              </tr>
              <tr v-if="!((rows ?? []).length)">
                <td colspan="5" class="py-10 text-center text-base-content/50">Belum ada data laba rugi pada periode ini.</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="grid gap-5 xl:grid-cols-2">
        <div class="ocn-panel">
          <div class="ocn-panel__head">
            <h2 class="ocn-panel__title">Pivot revenue vs beban</h2>
          </div>
          <div class="overflow-x-auto">
            <table class="table table-sm">
              ...
              <tbody>
                <tr v-for="row in type_pivot ?? []" :key="`${row.company_name}-${row.account_type}`">
                  <td>{{ row.company_name }}</td>
                  <td>
                    <span class="badge badge-sm" :class="row.account_type === 'revenue' ? 'badge-success' : 'badge-error'">
                      {{ row.account_type === 'revenue' ? 'Revenue' : 'Expense' }}
                    </span>
                  </td>
                  <td class="text-right font-semibold" :class="row.account_type === 'revenue' ? 'text-success' : 'text-error'">{{ format(row.amount) }}</td>
                </tr>
                <tr v-if="!((type_pivot ?? []).length)">
                  <td colspan="3" class="py-8 text-center text-base-content/50">Belum ada pivot laba rugi.</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="ocn-panel">
          <div class="ocn-panel__head">
            <h2 class="ocn-panel__title">Breakdown akun P&amp;L</h2>
          </div>
          <div class="overflow-x-auto">
            <table class="table table-sm">
              <thead>
              ...
              <tbody>
                <tr v-for="row in account_breakdown ?? []" :key="`${row.company_name}-${row.account_type}-${row.account_code}`">
                  <td>{{ row.company_name }}</td>
                  <td>
                ...
                <tr v-if="!((account_breakdown ?? []).length)">
                  <td colspan="4" class="py-8 text-center text-base-content/50">Belum ada breakdown akun.</td>
                </tr>
              </tbody>
            </table>
          </div>
