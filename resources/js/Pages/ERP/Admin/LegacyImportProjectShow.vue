        ...
        <div class="space-y-4">
          <div class="rounded-xl border border-base-200 bg-base-100 p-4">
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span class="badge badge-outline badge-sm">{{ project.payment_count }} payment</span>
            </div>
            <div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4 text-sm">
              <div class="rounded-lg border border-base-200 bg-base-200/30 p-3">
                <div class="text-xs uppercase tracking-[0.12em] text-base-content/60">Customer</div>
                <div class="mt-1 font-medium">{{ project.customer_name || '-' }}</div>
                <div v-if="project.crm_customer_match" class="mt-1 text-xs text-success">Match CRM: {{ project.crm_customer_match.name }}</div>
              </div>
              <div class="rounded-lg border border-base-200 bg-base-200/30 p-3">
                <div class="text-xs uppercase tracking-[0.12em] text-base-content/60">Tanggal jual</div>
                <div class="mt-1 font-medium">{{ project.sale_date || '-' }}</div>
                <div class="mt-1 text-xs text-base-content/60">{{ project.sale_date_source || '-' }}</div>
              </div>
              <div class="rounded-lg border border-base-200 bg-base-200/30 p-3">
                <div class="text-xs uppercase tracking-[0.12em] text-base-content/60">Nilai project</div>
                <div class="mt-1 font-medium">{{ Number(project.expected_value || 0).toLocaleString('id-ID') }}</div>
                <div class="mt-1 text-xs text-base-content/60">sumber {{ project.expected_value_source || '-' }}</div>
              </div>
              <div class="rounded-lg border border-base-200 bg-base-200/30 p-3">
                <div class="text-xs uppercase tracking-[0.12em] text-base-content/60">Total payment</div>
                <div class="mt-1 font-medium">{{ Number(project.paid_total || 0).toLocaleString('id-ID') }}</div>
                <div class="mt-1 text-xs text-base-content/60">{{ project.last_payment_date || '-' }}</div>
              </div>
            </div>
          ...
          <div v-if="project.issues?.length" class="rounded-xl border border-warning/40 bg-warning/10 p-4">
            ...
            <div class="mt-3 space-y-2 text-sm">
              <div v-for="issue in project.issues" :key="`${project.import_key}-${issue.code}`" class="rounded-lg border border-base-200 bg-base-100/80 px-3 py-2">
                <div class="flex flex-wrap items-center gap-2">
                  <span class="badge badge-sm" :class="issue.severity === 'error' ? 'badge-error' : 'badge-warning'">{{ issue.severity }}</span>
                  <span class="font-medium">{{ issue.label }}</span>
                </div>
                <div class="mt-1 text-base-content/80">{{ issue.message }}</div>
          ...
          <div class="grid gap-3 md:grid-cols-3 text-sm">
            <div class="rounded-xl border border-base-200 bg-base-100 p-4">
              <div class="text-xs uppercase tracking-[0.12em] text-base-content/60">Item</div>
              <div class="mt-2 text-2xl font-semibold">{{ project.compare_summary?.items_total ?? 0 }}</div>
              <div class="mt-1 text-xs text-base-content/60">Unresolved: {{ project.compare_summary?.items_unresolved ?? 0 }}</div>
            </div>
            <div class="rounded-xl border border-base-200 bg-base-100 p-4">
              <div class="text-xs uppercase tracking-[0.12em] text-base-content/60">Teknisi</div>
              <div class="mt-2 text-2xl font-semibold">{{ project.compare_summary?.technicians_total ?? 0 }}</div>
              <div class="mt-1 text-xs text-base-content/60">Unresolved: {{ project.compare_summary?.technicians_unresolved ?? 0 }}</div>
            </div>
            <div class="rounded-xl border border-base-200 bg-base-100 p-4">
              <div class="text-xs uppercase tracking-[0.12em] text-base-content/60">Payment teknisi</div>
              <div class="mt-2 text-2xl font-semibold">{{ project.compare_summary?.technician_payments_total ?? 0 }}</div>
              <div class="mt-1 text-xs text-base-content/60">Unresolved: {{ project.compare_summary?.technician_payments_unresolved ?? 0 }}</div>
            </div>
          </div>
          ...
          <div v-if="project.details?.technicians?.length" class="rounded-xl border border-base-200 bg-base-100 p-4">
            <div class="font-medium text-base-content">Teknisi project</div>
            <div class="mt-3 overflow-x-auto">
              <table class="table table-sm">
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr v-for="technician in project.details.technicians" :key="technician.legacy_assignment_id">
                    ...
                    <td><span class="badge badge-xs" :class="compareStatusClass(technician.status)">{{ technician.status }}</span></td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div v-if="project.details?.technician_payments?.length" class="rounded-xl border border-base-200 bg-base-100 p-4">
            <div class="font-medium text-base-content">Pembayaran teknisi</div>
            <div class="mt-3 overflow-x-auto">
              <table class="table table-sm">
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr v-for="payment in project.details.technician_payments" :key="payment.legacy_payment_id">
                    ...
                    <td><span class="badge badge-xs" :class="compareStatusClass(payment.status)">{{ payment.status }}</span></td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div class="space-y-4">
          <div class="rounded-xl border border-base-200 bg-base-100 p-4">
            ...
            <div class="mt-3 flex flex-col gap-2">
              ...
              <Link v-if="project.existing_erp_project?.id" class="btn btn-outline btn-sm justify-start" :href="route('projects.show', project.existing_erp_project.id)">
                Buka project ERP
              </Link>
            </div>
